 ...
 * 
 * $Id: mrcp_swift.c 1474 2010-02-07 20:51:47Z achaloyan $
 */
...
MRCP_PLUGIN_LOG_SOURCE_IMPLEMENT(CEPSTRAL_PLUGIN,"CEPSTRAL-PLUGIN")
/** Use custom log source mark */
#define CEPSTRAL_LOG_MARK   APT_LOG_MARK_DECLARE(CEPSTRAL_PLUGIN)
...
static apt_bool_t mrcp_swift_engine_open(mrcp_engine_t *engine)
{
	...
	/* open swift engine */
	apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"Open Swift Engine [%s]",swift_version);
	if((synth_engine->swift = swift_engine_open(NULL)) ==  NULL) {
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_WARNING,"Failed to Open Swift Engine");
		return FALSE;
	}

	if(mrcp_swift_voices_scan(synth_engine) == FALSE) {
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_WARNING,"Failed to Scan Swift Voices");
...
static apt_bool_t mrcp_swift_engine_close(mrcp_engine_t *engine)
{
	...
	/* close swift engine */
	apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"Close Swift Engine");
...
static apt_bool_t mrcp_swift_channel_open(mrcp_engine_channel_t *channel)
{
	...
	if(descriptor) {
		...
		/* open swift port */ 
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"Open Swift Port");
		...
		if(port) {
			...
			apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_WARNING,"Failed to Open Swift Port");
		}
	}
...
static apt_bool_t mrcp_swift_channel_close(mrcp_engine_channel_t *channel)
{
	...
	mrcp_swift_channel_t *synth_channel = channel->method_obj;
	apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"Close Swift Port");
...
static swift_result_t mrcp_swift_write_audio(swift_event *event, swift_event_t type, void *udata)
{
	...
	if(type & SWIFT_EVENT_END) {
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_DEBUG,"Swift Engine: Write End-of-Speech Event");
...
//if 0
		...
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_DEBUG,"Swift Engine: Write Audio [%d | %0.4f | %0.4f]",len, time_start, time_len);
//endif
...
static apt_bool_t mrcp_swift_channel_voice_set(mrcp_swift_channel_t *synth_channel, mrcp_message_t *message)
{
	...
	if(offset > 0) {
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"Find Swift Voice Matching the Criteria [%s]", search_criteria);
		voice = swift_port_find_first_voice(synth_channel->port,search_criteria,NULL);
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"Find Swift Voice [%s]", voice);

		if(!voice) {
			apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"No Swift Voice Available Matching the Criteria [%s]",search_criteria);
			/* find the first available one */
			voice = swift_port_find_first_voice(synth_channel->port,NULL,NULL);
		}

		if(voice) {
			apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"Set Swift Voice [%s]", swift_voice_get_attribute(voice, "name"));
			if(SWIFT_FAILED(res = swift_port_set_voice(synth_channel->port,voice)) ) {
				const char *error_string = swift_strerror(res);
				apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,error_string);
				return FALSE;
			}
		}
	}
	apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"mrcp_swift_channel_voice_set");
	return TRUE;
}
...
static apt_bool_t mrcp_swift_channel_param_set(mrcp_swift_channel_t *synth_channel, const char *name, swift_val *val)
{
	...
	if(SWIFT_FAILED(res = swift_port_set_param(synth_channel->port,name,val,synth_channel->tts_stream)) ) {
		const char *error_string = swift_strerror(res);
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_DEBUG,"Swift Param %s: %s",name,error_string);
		return FALSE;
	}
	return TRUE;
}
...
static apt_bool_t mrcp_swift_channel_params_set(mrcp_swift_channel_t *synth_channel, mrcp_message_t *message)
{
	...
	if(synth_header) {
		if(mrcp_resource_header_property_check(message,SYNTHESIZER_HEADER_PROSODY_VOLUME) == TRUE) {
			...
			if(swift_prosody_volume_get(&synth_header->prosody_param.volume,&volume) == TRUE) {
				name = "audio/volume";
				apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_DEBUG,"Swift Param %s=%d",name,volume);
		...
		if(mrcp_resource_header_property_check(message,SYNTHESIZER_HEADER_PROSODY_RATE) == TRUE) {
			...
			if(swift_prosody_rate_get(&synth_header->prosody_param.rate,&rate) == TRUE) {
				name = "speech/rate";
				apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_DEBUG,"Swift Param %s=%d",name,rate);
	...
	if(generic_header) {
		if(mrcp_generic_header_property_check(message,GENERIC_HEADER_CONTENT_TYPE) == TRUE) {
			...
			apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_DEBUG,"Swift Param %s=%s",name,generic_header->content_type.buf);
		...
		if(mrcp_generic_header_property_check(message,GENERIC_HEADER_CONTENT_ENCODING) == TRUE) {
			...
			apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_DEBUG,"Swift Param %s=%s",name,generic_header->content_encoding.buf);
...
static apt_bool_t mrcp_swift_voices_scan(mrcp_swift_engine_t *engine)
{
	...
	if(!engine->swift) {
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_WARNING,"Invalid Swift Engine");
		return FALSE;
	}
	...
	if((voice = swift_port_find_first_voice(port, NULL, NULL)) == NULL) {
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_WARNING,"No Swift Voice Available");
		swift_port_close(port);
		return FALSE;
	}
	/* go through all of the voices on the system and print some info about each */
	apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"Swift Available Voices:");
	for(; voice; voice = swift_port_find_next_voice(port)) {
		...
		apt_log(CEPSTRAL_LOG_MARK,APT_PRIO_INFO,"%s: %s, age %s, %s, %sHz, %s, %s",
